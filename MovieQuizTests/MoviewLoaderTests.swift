...
class MoviesLoaderTests: XCTestCase {
    func testSuccesLoading() throws {
        // Given
        let stubNetworkClient = StubMoviesNetworkClient(emulateError: false) // говорим что не хотим эмулировать ошибку
        ...
        loader.loadMovies { result in
            ...
            case .success(let movies):
                ...
                expectation.fulfill()
            case .failure(_):
                // мы не ожидаем, что пришла ошибка; если она появится, тест провалится
    ...
    func testFailureLoading() throws {
        // Given
        let stubNetworkClient = StubMoviesNetworkClient(emulateError: true)
        let loader = MoviesLoader(networkClient: stubNetworkClient)
